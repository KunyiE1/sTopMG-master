...
int TopPIC_identify(std::map<std::string, std::string> & arguments) {
  try {
    ...
    std::vector<std::string> input_exts;
      ...
      ModPtrVec mods_list = mod_util::readModTxt(var_ptm_file_name)[2];
      std::vector<std::string> mod_ori_res_list;
      std::vector<int> mods_mm_list;
      for(int i = 0; i < mods_list.size(); i++){
          mod_ori_res_list.push_back(mods_list[i]->getOriResiduePtr()->toString());
          mods_mm_list.push_back(round(mods_list[i]->getShift()*convert_ratio));
      }
...
//    input_exts.push_back("toppic_zero_shift_internal");
    ...
    clock_t filter_s, filter_e;
      clock_t search_s, search_e, search_e_1;
    filter_s = clock();
...
//      filter_e = clock();
//      std::cout<<"var ptm filter time: "<< (double)(filter_e - filter_s)/CLOCKS_PER_SEC << "s" << std::endl;
      ...
      var_ptm_search_mng_ptr->T_mass_comb_ = T_mass_comb;
      ...
      input_exts.push_back("toppic_var_ptm_internal");
        search_e_1 = clock();
        std::cout<<"search time: "<< (double)(search_e_1 - search_s)/CLOCKS_PER_SEC;
    }
...
//    }
    ...
    std::cout << "Top PrSM selecting - finished." << std::endl;
      search_e = clock();
      std::cout<<"search time: "<< (double)(search_e - search_s)/CLOCKS_PER_SEC;
    e = clock();
    std::cout<<"time: "<<double(e - s)/CLOCKS_PER_SEC<<"s"<<std::endl;
