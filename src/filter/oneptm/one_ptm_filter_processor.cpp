...
inline void filterBlock(const ProteoformPtrVec & raw_forms,
  ...
  //std::cout<<raw_forms.size()<<std::endl;
    start = clock();
  ...
  std::cout<<"preprocessing:"<<double(end - start)/CLOCKS_PER_SEC<<"s"<<std::endl;
  time_sum = 0;
...
//            end = clock();
//            temp_time = end - start;
//            std::cout<<"ASF on ptm time"<< double(temp_time)/CLOCKS_PER_SEC<<"s"<<std::endl;
        }

      }
        end = clock();
        //std::cout<<"1 spec filter time"<<double(end - start)/CLOCKS_PER_SEC<<"s"<<std::endl;
    }
//    std::cout<<"time = "<<double(time_sum)/CLOCKS_PER_SEC<<"s"<<std::endl;
//    std::cout<<"time = "<<double(time_sum)/CLOCKS_PER_SEC<<"s"<<std::endl;
...
std::function<void()> geneTask(int block_idx, 
  ...
  return[block_idx, mod_mass_list, mng_ptr] () {
    ...
    ProteoformPtrVec raw_forms
      ...
      std::cout<<"read seqeuences: "<< double(end - start)/CLOCKS_PER_SEC<<"s"<<std::endl;
...
void process(OnePtmFilterMngPtr mng_ptr) {
  ...
  for(int i = 0; i < mod_mass_list.size(); i++){
  std::cout<<"modification==="<<mod_mass_list[i]<<std::endl;
  }
