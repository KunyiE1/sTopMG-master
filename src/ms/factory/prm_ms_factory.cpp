...
void addTwoMasses(PrmPeakPtrVec &list, int spec_id, DeconvPeakPtr deconv_peak_ptr,
  ...
  //std::cout<<"active and Nterm "<<active_type_ptr->getName() <<',' << active_type_ptr->getN_BYShift()<< std::endl;
  PrmPeakPtr new_peak_ptr
      = std::make_shared<PrmPeak>(spec_id, deconv_peak_ptr,
                                  BasePeakType::ORIGINAL, n_term_mass, 1);
  new_peak_ptr->setStrictTolerance(tole_ptr->compStrictErrorTole(n_term_mass) + 0.1);
  ...
  //std::cout<<"active and Cterm "<<active_type_ptr->getName() <<',' << active_type_ptr->getC_BYShift() << std::endl;
  PrmPeakPtr reverse_peak_ptr
      = std::make_shared<PrmPeak>(spec_id, deconv_peak_ptr,
                                  BasePeakType::REVERSED, reverse_mass, 1);
  reverse_peak_ptr->setStrictTolerance(tole_ptr->compStrictErrorTole(reverse_mass + prec_mono_mass) + 0.1);
...
void addSixMasses(PrmPeakPtrVec &list, int spec_id, DeconvPeakPtr deconv_peak_ptr,
  ...
  for (size_t i = 0; i < offsets.size(); i++) {
    ...
    peak_ptr->setStrictTolerance(tole_ptr->compStrictErrorTole(mass) + 0.1);
  ...
  for (size_t i = 0; i < offsets.size(); i++) {
    ...
    peak_ptr->setStrictTolerance(tole_ptr->compStrictErrorTole(mass + prec_mono_mass) + 0.1);
...
PrmMsPtr geneMsTwoPtr(DeconvMsPtr deconv_ms_ptr, int spec_id, SpParaPtr sp_para_ptr,
  ...
  //std::cout<<"PREC"<<prec_mono_mass<<","<<new_prec_mono_mass<<std::endl;
  // getSpTwoPrmPeak
